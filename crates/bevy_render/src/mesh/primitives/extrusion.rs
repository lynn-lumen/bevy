...
/// A trait required to extrude 2d meshes.
///
/// To implement this trait, you have to implement [`Meshable`]
pub trait Extrudable {
    /// Get the indices of the meshs perimeter
    fn perimeter_indices(&self) -> Vec<Indices>;
}

pub struct ExtrusionMeshBuilder<T: Primitive2d + Extrudable + Meshable<Output = Mesh>> {
    base_shape: T,
    half_depth: f32,
    segments: usize,
}
impl<T: Primitive2d + Extrudable + Meshable<Output = Mesh>> ExtrusionMeshBuilder<T> {
    pub fn segments(mut self, segments: usize) -> Self {
        self.segments = segments;
        self
    }
}
impl<T: Primitive2d + Extrudable + Meshable<Output = Mesh>> MeshBuilder
    for ExtrusionMeshBuilder<T>
{
    fn build(&self) -> Mesh {
        build_extrusion(
            self.base_shape.mesh(),
            self.base_shape.perimeter_indices(),
            self.half_depth,
            self.segments,
        )
    }
}
impl<T> Meshable for Extrusion<T>
where
    T: Meshable<Output = Mesh> + Primitive2d + Extrudable + Clone,
{
